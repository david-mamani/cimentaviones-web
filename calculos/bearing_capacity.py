...
def _extreme_pressures_trapezoidal(
    Q: float, B: float, L: float, e1: float, e2: float
) -> dict:
    """
...
def _enrich_pressure(
    qu: float, FS: float, B_for_area: float, L_for_area: float
) -> dict:
...
def _build_method_block(
    method_result: dict, soil_type: str, FS: float, B_for_area: float, L_for_area: float
) -> dict:
    """
...
def calculate_bearing_capacity(input_data: dict) -> dict:
    ...
    M1 = foundation.get("M1")
    M2 = foundation.get("M2")
    Q_applied = foundation.get("Q")
    ...
    kern_metric = 0.0
    if has_eccentricity:
        kern_metric = (6.0 * e1 / L if L > 0 else 0.0) + (
            6.0 * e2 / B if B > 0 else 0.0
        )
