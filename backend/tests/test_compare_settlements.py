...
# Permitir `from main import app` desde backend/tests/.
...
def _base_footing(id_: str, B: float, L: float, Q: float):
    return {
        ...
        "foundation": {
            ...
            "B": B, "L": L, "Df": 1.0, "FS": 3.0, "beta": 0.0,
            "Q": Q,
        },
        "strata": [
            {"id": "s1", "thickness": 2.0, "gamma": 18.0, "gammaSat": 20.0,
             "c": 0.0, "phi": 30.0, "Es": 15000, "mu_s": 0.30},
            {"id": "s2", "thickness": 15.0, "gamma": 18.0, "gammaSat": 20.0,
             "c": 0.0, "phi": 30.0, "Es": 20000, "mu_s": 0.30},
        ],
        "conditions": {"hasWaterTable": False, "waterTableDepth": 0.0,
                       "hasBasement": False, "basementDepth": 0.0},
        "settlement": {"S_max": 0.05, "point": "centro", "rigid": False,
                       "Cw_method": "off", "consolidation": False},
    }
...
class TestCompareSettlementsEndpoint:
    ...
    def test_three_footings_finds_worst_pair(self):
        # 3 zapatas con cargas crescientes ⇒ peor par debe ser Z1-Z3.
        body = {
            "footings": [
        ...
        assert r.status_code == 200, r.text
        data = r.json()
        # δ_13 debe ser el mayor en absoluto (mayor diferencia de carga)
        # pero β depende del span; verificamos al menos que se identifica un par.
        assert data["worst"]["beta"] > 0
        # Δ entre Z1 y Z3 debe ser el mayor
    ...
    def test_invalid_diagonal_nonzero(self):
        body = {
            ...
            "spans": [[1, 5.8], [5.8, 0]],   # diagonal no cero
        }
        r = client.post("/api/compare-settlements", json=body)
    ...
    def test_invalid_asymmetric_spans(self):
        body = {
            ...
            "spans": [[0, 5.8], [3.0, 0]],   # no simétrica
        }
        r = client.post("/api/compare-settlements", json=body)
        assert r.status_code == 422
    ...
    def test_duplicate_ids_rejected(self):
        body = {
            "footings": [
                _base_footing("Z1", 1.6, 2.0, 630.0),
                _base_footing("Z1", 1.6, 2.0, 750.0),   # id duplicado
            ],
            "spans": [[0, 5.8], [5.8, 0]],
        }
        r = client.post("/api/compare-settlements", json=body)
    ...
    def test_missing_Q_rejected(self):
        ...
        f2["foundation"].pop("Q")  # quitar Q ⇒ no se puede comparar S_total
