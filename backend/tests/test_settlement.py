...
class TestRigidDetection:
    def test_detects_when_Es_jumps_10x(self):
        ...
        strata = [
            ...
            {"thickness": 2.0, "Es": 10000},
            {"thickness": 2.0, "Es": 150000},
...
class TestMuSelection:
    def test_picks_layer_below_base(self):
        strata = [
            {"thickness": 1.0, "mu_s": 0.20},
            {"thickness": 5.0, "mu_s": 0.35},
        ]
...
class TestCw:
    def test_no_water_table(self):
        r = water_table_correction(
            Dw=0, Df=1, B=1, method="peck", has_water_table=False
        )
        assert r["Cw"] == 1.0
    ...
    def test_peck_below_base(self):
        ...
        r = water_table_correction(
            Dw=1.5, Df=1.0, B=1.0, method="peck", has_water_table=True
        )
    ...
    def test_Dw_above_base_forces_Cw_2(self):
        ...
        r = water_table_correction(
            Dw=0.5, Df=1.0, B=1.0, method="peck", has_water_table=True
        )
        assert r["Cw"] == 2.0
    ...
    def test_teng_clamped_to_2(self):
        ...
        r = water_table_correction(
            Dw=1.2, Df=1.0, B=1.0, method="teng", has_water_table=True
        )
        assert r["Cw"] <= 2.0

    def test_off_method(self):
        r = water_table_correction(
            Dw=1.5, Df=1.0, B=1.0, method="off", has_water_table=True
        )
        assert r["Cw"] == 1.0
...
class TestQadmSettlement:
    def test_inverse_consistency(self):
        ...
        se = elastic_settlement(
            q0_net=q["qadm_settlement"], **params, point="centro", rigid=False
        )
    ...
    def test_with_Cw_2_halves_qadm(self):
        ...
        assert q2["qadm_settlement"] == pytest.approx(
            q1["qadm_settlement"] / 2.0, rel=1e-9
        )
...
class TestConsolidation:
    def test_NC_case(self):
        r = consolidation_settlement(
            Hc=2.0, e0=0.8, Cc=0.3, sigma_p0=100.0, dsigma_av=50.0
        )
        assert r["case"] == "NC"
...
class TestBjerrum:
    def test_1_over_600_in_table(self):
        ...
        assert DISTORTION_LIMITS_RNE_E050[
            "peligro_porticos_diagonales"
        ] == pytest.approx(1.0 / 600, abs=1e-9)
