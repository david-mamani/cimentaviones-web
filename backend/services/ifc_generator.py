...
FOUNDATION_COLOR = (0.498, 0.549, 0.553)
WATER_COLOR = (0.204, 0.596, 0.859)
PAD_HEIGHT = 0.3
COLUMN_WIDTH_RATIO = 0.3
COLUMN_WIDTH_MAX = 0.5
SOIL_PADDING = 2.0
WATER_TABLE_THICKNESS = 0.02
...
def _create_cylinder_shape(ifc_file, context, radius, height):
    ...
    x_dir = ifc_file.createIfcDirection([1.0, 0.0])
    placement_2d = ifc_file.createIfcAxis2Placement2D(point, x_dir)
    profile = ifc_file.createIfcCircleProfileDef("AREA", None, placement_2d, radius)
    extrusion_dir = _create_direction(ifc_file, (0.0, 0.0, 1.0))
    solid = ifc_file.createIfcExtrudedAreaSolid(
        profile,
        _create_axis2placement3d(ifc_file, (0.0, 0.0, 0.0)),
...
def generate_ifc(
    ...
    building = ifc_file.createIfcBuilding(
        ...
        owner_history,
        "Cimentación",
        "Estructura de cimentación superficial",
        None,
        building_placement,
        None,
        None,
    ...
    basement_depth = (
        conditions.get("basementDepth", 0) if conditions.get("hasBasement") else 0
    )
    if f_type == "cuadrada":
        L = B
    soil_w = B + 4
    ...
    for i, stratum in enumerate(strata):
        ...
        placement = _create_local_placement(
            ifc_file, storey_placement, origin=(0.0, 0.0, -(z_offset + h))
        )
    ...
    pad_placement = _create_local_placement(
        ifc_file, storey_placement, origin=(0.0, 0.0, -total_depth)
    )
    if f_type == "circular":
    ...
    footing = ifc_file.createIfcFooting(
        ...
        owner_history,
        "Zapata",
        f"Cimentación {f_type} — B={B}m, L={L}m, Df={Df}m",
        None,
        pad_placement,
        pad_shape,
        None,
    ...
    if col_height > 0:
        ...
        column = ifc_file.createIfcColumn(
            ...
            owner_history,
            "Pedestal",
            f"Pedestal de la zapata — {col_w:.2f}m × {col_w:.2f}m",
            None,
            col_placement,
            col_shape,
            None,
    ...
    if conditions.get("hasWaterTable"):
        ...
        wt_placement = _create_local_placement(
            ifc_file, storey_placement, origin=(0.0, 0.0, -(wt_depth + wt_thickness))
        )
        wt_shape = _create_box_shape(
            ifc_file, sub_context, wt_size, wt_size, wt_thickness
        )
    ...
    ifc_file.createIfcRelContainedInSpatialStructure(
        ifcopenshell.guid.new(), owner_history, None, None, products, storey
    )
