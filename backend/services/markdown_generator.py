def generate_resolution_md(input_data: dict, result: dict, unit_config: dict = None) -> str:
    ...
    gamma_eff = _cout(result["gammaEffective"], iu_weight)
    ...
    geom = f"$\\rightarrow B = {B:.2f}$ {iu_length}, $\\quad L = {L:.2f}$ {iu_length}, $\\quad D_f = {Df:.2f}$ {iu_length}" if f_type == "rectangular" else f"$\\rightarrow B = {B:.2f}$ {iu_length}, $\\quad D_f = {Df:.2f}$ {iu_length}"
