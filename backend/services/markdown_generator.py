...
def generate_resolution_md(
    input_data: dict, result: dict, unit_config: dict = None
) -> str:
    """
    ...
    md.append("")
    md.append(
        f"$$ c = {c:.2f} \\text{{ {u_pres}}}, \\quad \\phi = {phi:.2f}^\\circ, \\quad \\gamma = {gamma:.2f} \\text{{ {u_weight}}}, \\quad \\gamma_{{sat}} = {gammaSat:.2f} \\text{{ {u_weight}}} $$"
    )
    md.append("")
    ...
    md.append("")
    md.append(
        f"$$ N_c = {Nc:.3f}, \\quad N_q = {Nq:.3f}, \\quad N_\\gamma = {Ng:.3f} $$"
    )
    md.append("")
    md.append("#### 3. Factores de Modificación")
    md.append("")
    md.append(
        "| Parámetro | Cohesión ($c$) | Sobrecarga ($q$) | Peso Específico ($\\gamma$) |"
    )
    md.append("| :--- | :---: | :---: | :---: |")
    md.append(
        f"| **Forma ($s$)** | $s_c = {sc:.3f}$ | $s_q = {sq:.3f}$ | $s_\\gamma = {sg:.3f}$ |"
    )
    if method == "general":
        md.append(
            f"| **Profundidad ($d$)** | $d_c = {dc:.3f}$ | $d_q = {dq:.3f}$ | $d_\\gamma = {dg:.3f}$ |"
        )
    if beta > 0 or method == "general" or method == "rne":
        md.append(
            f"| **Inclinación ($i$)** | $i_c = {ic:.3f}$ | $i_q = {iq:.3f}$ | $i_\\gamma = {ig:.3f}$ |"
        )
    md.append("")
    ...
    md.append("| :--- | :---: | :---: | :---: |")
    md.append(
        f"| **Capacidad Neta Última** ($q_u - q$) | $q_{{net}}$ | **{qnet:.2f}** | {u_pres} |"
    )
    md.append(
        f"| **Capacidad Admisible** ($q_u / FS$) | $q_{{adm}}$ | **{qa:.2f}** | {u_pres} |"
    )
    md.append(
        f"| **Capacidad Neta Admisible** ($q_{{net}} / FS$) | $q_{{net(adm)}}$ | **{qa_net:.2f}** | {u_pres} |"
    )
    md.append(
        f"| **Carga Máxima Estructural** | $Q_{{max}}$ | **{Qmax:.2f}** | {u_force} |"
    )
    md.append("")
    ...
    if rne:
        ...
        md.append("")
        md.append(
            "Reajuste según estipulaciones locales del Reglamento Nacional de Edificaciones:"
        )
        md.append("")
        md.append(
            f"$$ q_{{u(RNE)}} = {quRNE:.2f} \\text{{ {u_pres}}}, \\quad q_{{adm(RNE)}} = {qaRNE:.2f} \\text{{ {u_pres}}} $$"
        )
        md.append("")
