...
class Stratum(BaseModel):
    ...
    Es: Optional[float] = Field(
        default=None, gt=0, description="Módulo de elasticidad (kPa)"
    )
    mu_s: Optional[float] = Field(default=None, ge=0.0, le=0.5, description="Poisson")
    is_clay: bool = Field(
        default=False, description="Marca estrato como arcilla (activa Sc)"
    )
    Cc: Optional[float] = Field(default=None, gt=0, description="Índice de compresión")
    Cs: Optional[float] = Field(
        default=None, gt=0, description="Índice de recompresión"
    )
    e0: Optional[float] = Field(
        default=None, gt=0, description="Relación de vacíos inicial"
    )
    sigma_c: Optional[float] = Field(
        default=None, gt=0, description="σ'c preconsolidación (kPa)"
    )
    ...
    def validate_gammaSat_gte_gamma(self):
        if self.gammaSat < self.gamma:
            raise ValueError(f"γsat ({self.gammaSat}) debe ser ≥ γ ({self.gamma})")
        return self
...
class FoundationParams(BaseModel):
    ...
    e1: float = Field(
        default=0.0, ge=0, description="Excentricidad por M1/Q — reduce L (m)"
    )
    e2: float = Field(
        default=0.0, ge=0, description="Excentricidad por M2/Q — reduce B (m)"
    )
...
class IterationConfig(BaseModel):
    ...
    lbRatio: Optional[float] = Field(
        default=None, gt=0, description="L = lbRatio × B (for locked rectangular)"
    )

    @model_validator(mode="after")
    def validate_ranges(self):
        if self.varyB and self.bEnd < self.bStart:
            raise ValueError(f"bEnd ({self.bEnd}) debe ser ≥ bStart ({self.bStart})")
...
class SettlementParams(BaseModel):
    ...
    def validate_t1_t2(self):
        if self.t1 is not None and self.t2 is not None and self.t2 <= self.t1:
            raise ValueError(f"t2 ({self.t2}) debe ser > t1 ({self.t1}) para Sc_s.")
        return self
...
class SettlementIterationInput(BaseModel):
    ...
    def validate_range(self):
        if self.B_end < self.B_start:
            raise ValueError(
                f"B_end ({self.B_end}) debe ser ≥ B_start ({self.B_start})"
            )
        return self
...
class NamedSettlementInput(SettlementInput):
    ...
    id: str = Field(
        min_length=1, description="Identificador único de la zapata (ej. 'Z1')"
    )
...
class PDFExportInput(BaseModel):
    ...
    unit_config: Optional[PDFUnitConfig] = None
